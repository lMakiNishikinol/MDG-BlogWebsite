...
const TargetCursor: React.FC<TargetCursorProps> = ({
  ...
  useEffect(() => {
    ...
    let _debugAfterRelease = 0;

    const enterHandler = (e: MouseEvent, overrideTarget?: Element) => {
      const directTarget = overrideTarget || (e.target as Element);
      ...
      let target: Element | null = allTargets[0] || null;

      // 后备方案：DOM 向上遍历没找到时，用 elementFromPoint 查找
      if (!target) {
        const pointEl = document.elementFromPoint(e.clientX, e.clientY);
        if (pointEl) {
          target = pointEl.closest(targetSelector);
        }
      }

      if (!target || !cursorRef.current || !cornersRef.current) {
        if (_debugAfterRelease > 0) {
          console.log("[Cursor] enterHandler: no target found. directTarget=", directTarget?.tagName, directTarget?.className?.substring(0, 80));
        }
        return;
      }
      ...
      if (_debugAfterRelease > 0) {
        _debugAfterRelease--;
        console.log("[Cursor] enterHandler: snapping to", target.tagName, target.className?.substring(0, 80));
      }
    ...
    const moveHandler = (e: MouseEvent) => {
      ...
      // 自愈机制：当无吸附目标时，用 elementFromPoint 检测鼠标下方是否有 cursor-target
      if (!activeTarget) {
        const hitEl = document.elementFromPoint(e.clientX, e.clientY);
        const snapTarget = hitEl?.closest?.(targetSelector);
        if (_debugAfterRelease > 0) {
          console.log("[Cursor] heal check:", hitEl?.tagName, hitEl?.className?.substring(0, 80), "snap=", !!snapTarget);
        }
        if (snapTarget) {
          enterHandler(e, snapTarget);
        }
      }
    };
    window.addEventListener("mousemove", moveHandler);

    // 监听自定义事件，强制释放当前磁吸目标（如灯箱打开时）
    const cursorReleaseHandler = () => {
      console.log("[Cursor] RELEASE: active=", activeTarget?.tagName, "handler=", !!currentLeaveHandler);
      if (activeTarget && currentLeaveHandler) {
        currentLeaveHandler();
      }
      _debugAfterRelease = 10;
      console.log("[Cursor] RELEASE done: active=", activeTarget?.tagName);
    };
    window.addEventListener("cursor:release", cursorReleaseHandler);

    return () => {
      window.removeEventListener("mousemove", moveHandler);
      window.removeEventListener("mouseover", enterHandler);
      window.removeEventListener("scroll", scrollHandler);
      window.removeEventListener("cursor:release", cursorReleaseHandler);
  ...
  return (
    <div
      ref={cursorRef}
      className="fixed top-0 left-0 w-0 h-0 pointer-events-none z-[200000] mix-blend-difference transform -translate-x-1/2 -translate-y-1/2 touch-coarse:hidden"
